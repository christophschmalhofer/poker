...
__int64 SaveID(__int64 ID) 
{
   auto ret = subHands.insert(make_pair(ID,0));
   if(ret.second) {
      subHandQueue.push_back(ID);
   }
}
...
int _tmain(int argc, _TCHAR* argv[])
{
   ...
   {   
      ...
      {	
	 __int64 aCardMore = MakeID(subHand, card);   // get the new ID for it
	 if (aCardMore == 0) {
	    continue; //impossible hand
	 }
	 if (numcards < 7) {
	    // JMD: and save it in the list if I am not on the 7th card
	    auto ret = subHands.insert(make_pair(aCardMore,0));
	    if(ret.second) {
	       subHandQueue.push_back(aCardMore); //not yet processed -> add to queue
	    }
	 } else {
	    break;
	 }
   ...
   for(auto it = subHands.begin(); it != subHands.end(); ++it, subHandPosition++) {
	 ...
	 __int64 aCardMore = MakeID(subHand, card);
